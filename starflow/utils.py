...
def print_timing(func):
    ...
    def wrapper(*arg, **kargs):
        ...
        res = func(*arg, **kargs)
        time2 = time.time()
        log.info('%s took %0.3f mins' % (func.__name__, (time2-time1)/60.0))
        return res
    return wrapper
...
def annotator(f,x,y,*z):
    if x != None:
        f = single_annotator(f, '__depends_on__', x)
    if y != None:
        f = single_annotator(f, '__creates__', y)
    if z != ():
        f = single_annotator(f, '__uses__', z[0])   # check this ...
    return f

def depends_on(x):
    return lambda f: single_annotator(f, '__depends_on__', x)

def creates(x):
    return lambda f: single_annotator(f, '__creates__', x)

def uses(x):
    return lambda f: single_annotator(f, '__uses__', x)

def single_annotator(f, attr_name, g):
    '''
    f is a function with only default arguments
    attr_name is a string in ['__depends_on__', '__creates__', '__uses__']
    g is a lambda function mapping a tuple of f's arguments to an annotation
    '''
    args = inspect.getargspec(f).defaults
    f = append_decorated_attribute(f, attr_name, g(args))
    return f

def append_decorated_attribute(f, attr_name, attr_val):
    attr_val = MakeT(attr_val)
    if hasattr(f, attr_name):
        setattr(f, attr_name, MakeT(getattr(f, attr_name)) + attr_val)
    else:
...
def copy_contents(src,dest):
    L = listdir(src)
    for l in L:
        recursive_copy(os.path.join(src,l),dest)
...
def recursive_copy(src,dest):
    if os.path.isfile(src):
        ...
        if os.path.isdir(dest):
            [outer,name] = os.path.split(os.path.normpath(src))
