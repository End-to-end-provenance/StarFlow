...
link structure local to a give path in the Data Environment.
...
def MakeLocalLinkList(Path,depends_on = os.path.join(WORKING_DE.relative_links_dir,'StoredTimes'), creates = (WORKING_DE.relative_metadata_dir,)):
    ...
    if not IsFile(LinkPath) or not IsFile(LiveModulePath):
        ...
        StoredFileList = pickle.load(open(LiveModulePath,'rb'))
        ...
        if not all([PathExists(x) and x in FileList for x in ExistingScriptFiles]) or not isinstance(StoredFileList,dict):
            ...
            SeedFiles = [x for x in FileList if not x.split('/')[-1] == '__init__.py' and (x not in list(StoredFileList.keys()) or os.path.getmtime(x) > StoredFileList[x])]

    F = open(LiveModulePath,'wb')
    SFileDict = dict([(x[0],x[1]) for x in numpy.load(os.path.join(WORKING_DE.links_dir,'StoredTimes')) if x[0] in FileList]) #script and time
    pickle.dump(SFileDict,F)
    F.close()
...
def MakeLocalLinkGraph(Path,Mode='ColDir',ShowUses='No',ShowImplied='No'):
    ...
    if not PathExists(LinkPath):
        ...
        if min([T,T3,T4]) < T2:
            ...
            # loading empty. nothing in the pathpickle. check in MakeLocalLinkList
            G = pickle.load(open(metapathpickle,'rb'))
            return ['',metapath,G,metapathhtml]
...
def MakeLinkListHtml(LinkList,ClusterTags,inpath,outpath,Mode,ShowUses,ShowImplied):
    ...
    for i in range(len(LinkList)):
        ...
        Recs.append([LinkList['LinkType'][i],slink,splink,tlink,tplink,ClusterTags[s] if s in list(ClusterTags.keys()) else '',ClusterTags[t] if t in list(ClusterTags.keys()) else ''])
...
def LabeledGraphFromLinks(LinkList,Path,ClusterTags=None,Mode='ColDir',ShowUses='No',ShowImplied='No'):
    ...
    A = numpy.rec.fromrecords(uniqify(list(zip(LinkList['LinkSource'],LinkList['SourceFile'],CB,FB)) + list(zip(LinkList['LinkTarget'],LinkList['TargetFile'],DO,FO))), names = ['Object','File','OpType','FuncType'])

    CTags = dict([(a,a) for a in A['Object'] if a not in list(ClusterTags.keys())])
    ...
    X = list(zip(LinkList['LinkSource'], LinkList['LinkTarget']))
    ...
    NodeAttrs= {}
    for n in Nodes:
        NodeAttrs[n] = dict(list(zip(['color','style','shape','label','width','height','target','URL'],NodePropertiesSelector(n,Path,ClusterTags,NodeInfo,Mode,ShowUses,ShowImplied))))

    EdgeAttrs = {}
    for e in Edges:
        EdgeAttrs[e] = dict(list(zip(['color','labelfloat','label','style'],EdgePropertiesSelector(e,EdgeInfo))))
...
def NodePropertiesSelector(n,Path,ClusterDict,NodeInfo,Mode,ShowUses,ShowImplied):
    ...
    IsCluster = n in list(ClusterDict.values()) and len(NodeInfo[n]) > 1
...
def ProcessTwoDicts(A,B):
    ...
    '''
    T = [(l,A[l] + B[l] if (l in list(A.keys()) and l in list(B.keys())) else A[l] if l in list(A.keys()) else B[l]) for l in set(A.keys()).union(list(B.keys()))]
...
def GetClusterTagDict(Path,LinkList,Mode):
    ...
    if Mode == 'All':
        ...
        if IsDir(Path):
            ...
            for ii in range(2):
                SClusterDict2 = dict([(LS['LinkSource'][LSDiffs[i]],[ClusterDict[l] for l in LS['LinkTarget'][LSDiffs[i]:LSDiffs[i+1]] if l in list(ClusterDict.keys())]) for i in range(len(LSDiffs)-1) if LS['LinkSource'][LSDiffs[i]] not in list(ClusterDict.keys())])
                TClusterDict2 = dict([(LT['LinkTarget'][LTDiffs[i]],[ClusterDict[l] for l in LT['LinkTarget'][LTDiffs[i]:LTDiffs[i+1]] if l in list(ClusterDict.keys())]) for i in range(len(LTDiffs)-1) if LT['LinkTarget'][LTDiffs[i]] not in list(ClusterDict.keys())])
                ClusterDict2 = ProcessTwoDicts(SClusterDict2,TClusterDict2)
                C2K = numpy.array(list(ClusterDict2.keys()))
                C2V = numpy.array(list(ClusterDict2.values()))
                s = C2V.argsort()
                C2V = C2V[s]
                C2K = C2K[s]
                Diffs = numpy.append(numpy.append([-1],(C2V[1:] != C2V[:-1]).nonzero()[0]),[len(C2V)])
                NewTags = dict(list(zip(C2K.tolist(),ListUnion([[MaximalCP(C2K[Diffs[i]+1:Diffs[i+1]+1])]*(Diffs[i+1] - Diffs[i]) for i in range(len(Diffs)-1)]))))
                ClusterDict.update(NewTags)
            if len(OutSides) > .1*len(uniqify(list(ClusterDict.values()))) and len(OutSides) > 4:
        ...
        for target in Targets:
            ...
            for metapath in metapaths:
                if PathExists(metapath):
                    ...
                    if isinstance(MetaData,dict) and 'ProtocolTags' in list(MetaData.keys()):
...
def WriteOutGraphDot(G,outpath):
    ...
    for node in Nodes:
        S += '"' + str(node) + '"' + '  [' + ','.join([att + '="' + NodeAttrs[node][att] + '"' for att in list(NodeAttrs[node].keys())]) + ']\n'

    for edge in Edges:
        S += '"' + str(edge[0]) + '"' + ' -> ' + '"' + str(edge[1]) + '"    [' + ','.join([att + '="' + EdgeAttrs[edge][att] + '"' for att in list(EdgeAttrs[edge].keys())]) + ']\n'
...
def MostCommonValue(D):
    ...
    argmax = list(countdict.values()).index(m)
    return list(countdict.keys())[argmax]
