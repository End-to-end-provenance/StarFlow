...
def aggregate(X, On=None, AggFuncDict=None, AggFunc=None,
    ...
    **See also:**
    ...
    if AggFuncDict:
        AggList += list(AggFuncDict.items())
...
def strictaggregate(X,On,AggList,returnsort=False, keyfuncdict=None):
    ...
    if len(On) > 0:
        ...
        if keyfuncdict is not None:
            for _kf in keyfuncdict:
                fn = keyfuncdict[_kf]
                keycols[_kf] = np.array(list(map(fn, keycols[_kf])))
    ...
    isinstance(AggColDict[AggList[0][0]][0],np.ndarray):
        lens = list(map(len, AggColDict[AggList[0][0]]))
        OnCols = OnCols.repeat(lens)
        for o in list(AggColDict.keys()):
        ...
        for i in range(len(Diffs)-1):
            ...
            for l in list(HH.keys()):
                Get += [len(X) + j for j in
                        HH[l][2][list(range(HH[l][0][i], HH[l][1][i]))] if
...
def pivot(X, a, b, Keep=None, NullVals=None, order = None, prefix='_'):
    ...
    **Returns**
    ...
    if len(X) != Da * Db:
        ...
        nullvals = utils.fromrecords([[NullVals[o] if o in list(NullVals.keys())
...
def join(L, keycols=None, nullvals=None, renamer=None,
    ...
    **See Also:**
    ...
    if len(commons) > 0:
        ...
        NewNames = [[l if l not in list(renaming[k].keys()) else renaming[k][l]
...
def strictjoin(L, keycols, nullvals=None, renaming=None, Names=None):
    ...
    **See Also:**
    ...
    for i in Names:
        ...
        if i in list(renaming.keys()):
            for k in list(renaming[i].keys()):
                if k not in keycols:
...
def RenamingIsInCorrectFormat(renaming, L, Names=None):
    ...
    return isinstance(renaming, dict) and \
           ...
           all([isinstance(renaming[k],dict) and
                set(renaming[k].keys()) <=
                set(LL[Names.index(k)].dtype.names) for k in list(renaming.keys())])
