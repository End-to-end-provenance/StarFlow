...
def tab_colstack(ListOfTabArrays, mode='abort'):
    ...
    for (i, a) in enumerate(ListOfTabArrays):
        ...
        for k in a.coloring:
            ...
            if k in list(coloring.keys()):
                coloring[k] = utils.uniqify(coloring[k] + s)
            else:
                coloring[k] = s

    for k in list(coloring.keys()):
...
def tab_join(ToMerge, keycols=None, nullvals=None, renamer=None,
    ...
    if isinstance(ToMerge,dict):
        Names = list(ToMerge.keys())
    else:
        Names = list(range(len(ToMerge)))

    Colorings = dict([(k,ToMerge[k].coloring) if 'coloring' in dir(ToMerge[k])
                                              else {} for k in Names])
    for k in Names:
        if k in list(Renaming.keys()):
            l = ToMerge[k]
            Colorings[k] = \
                dict([(g, [n if not n in list(Renaming[k].keys()) else Renaming[k][n]
                       for n in l.coloring[g]]) for g in list(Colorings[k].keys())])
    Coloring = {}
    for k in list(Colorings.keys()):
        for j in list(Colorings[k].keys()):
            if j in list(Coloring.keys()):
...
class tabarray(np.ndarray):
     ...
     **Special column names:**
        ...
        if not array is None:
            ...
            if (names is None) and 'names' in list(metadata.keys()) and metadata['names']:
                names = metadata['names']
            if (coloring is None) and 'coloring' in list(metadata.keys()) and metadata['coloring']:
    ...
    def __getitem__(self, ind):
        ...
        *	If you pass a :class:`numpy.ndarray`, you get a tabarray consisting
        ...
        """
        if ind in list(self.coloring.keys()):
            return self[self.coloring[ind]]
        elif isinstance(ind,list) and self.dtype.names and \
             all([a in self.dtype.names or a in list(self.coloring.keys())
                                                           for a in ind]) and \
            ...
            if isinstance(D, np.ndarray) and not (D.dtype.names is None):
                ...
                for k in list(self.coloring.keys()) if
    ...
    def renamecol(self, old, new):
        ...
        for x in list(self.coloring.keys()):
    ...
    def replace(self, old, new, strict=True, cols=None, rows=None):
        ...
        replace.__doc__ = modifydocs(replace, spreadsheet.replace, ":func:`tabular.spreadsheet.replace`")
    ...
    def join(self, ToMerge, keycols=None, nullvals=None,
        ...
        **See also:** :func:`tabular.spreadsheet.join`, :func:`tab_join`
        ...
        if isinstance(ToMerge,dict):
            assert selfname not in list(ToMerge.keys()), \
