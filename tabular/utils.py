...
def fromarrays(X, type=None, **kwargs):
    if 'dtype' in list(kwargs.keys()) and kwargs['dtype'] and 'object':
        _array = dtypecolumnloader(X, **kwargs)
    else:
        try:
            _array = np.rec.fromarrays(X, **kwargs)
        except (TypeError,ValueError):
            if 'formats' in list(kwargs.keys()) and kwargs['formats']:
...
def fromrecords(X, type=None, **kwargs):
    if 'dtype' in list(kwargs.keys()) and kwargs['dtype']:
            ...
            if 'formats' in list(kwargs.keys()) and kwargs['formats']:
                ...
                if 'str' not in formats:
                    dtype = getdtype(kwargs['names']
                          if 'names' in list(kwargs.keys()) else None, formats)
...
def fromkeypairs(kvpairs,dtype=None,fillingvalues=None):
    ...
    if not dtype:
        dtype = np.dtype(list(zip(names,realtypes)))
        ...
        if fillingvalues:
            if isinstance(fillingvalues,list) or \
               ...
               vfd = dict([(n,
                ...
                intkeys = [k for k in list(fillingvalues.keys()) if isinstance(k,int)]
                vfd = dict([(k, v) for (k, v) in list(fillingvalues.items())
                                 if k in intkeys])
                strkeys = [k for k in list(fillingvalues.keys()) if isinstance(k,str)]
                if len(strkeys) > 0:
                    namedict = dict([(names[j],j) for j in numbers])
                    for n in strkeys:
                        vfd[namedict[n]] = fillingvalues[n]
                for k in numbers:
                    if k not in list(vfd.keys()):
                        vfd[k] = None
            else:
...
def getdtype(names, formats):
    return np.dtype(list(zip(names,formats))) if names != None else np.dtype(formats)
