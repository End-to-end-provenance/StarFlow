...
def loadSV(fname, shape=None, titles=None, aligned=False, byteorder=None,
    ...
    **See Also:**
    ...
    if 'names' in list(metadata.keys()):
        names = metadata['names']
    else:
        names = None

    if 'formats' in list(metadata.keys()):
        formats = metadata['formats']
    else:
        formats = None

    if 'dtype' in list(metadata.keys()):
    ...
    if renamer is not None:
        ...
        if len(renamed) == len(uniqify(renamed)):
            names = renamed
            print ('''... using renamed names (original names will be in return metadata)''')
        else:
...
def loadSVcols(fname, usecols=None, excludecols=None, valuefixer=None,
    ...
    **See Also:**
    ...
    if usecols is not None:
        ...
        if 'names' in list(metadata.keys()):
            ...
            if 'coloring' in list(metadata.keys()):
                ...
                for c in usecols:
                    if c in list(coloring.keys()):
                        getcols += [names.index(n) for n in coloring[c]]
        getcols = uniqify(getcols)
    else:
        if 'names' in list(metadata.keys()):
            names = metadata['names']
            getcols = list(range(len(names)))
        else:
            getcols = list(range(l0))
    ...
    if 'formats' in list(metadata.keys()) or 'types' in list(metadata.keys()):
        if 'formats' in list(metadata.keys()):
            formats = metadata['formats']
        else:
            formats = metadata['types']
        formats = dict(list(zip(getcols,formats)))
    else:
...
def loadSVrecs(fname, uselines=None, skiprows=0, linefixer=None,
    ...
    **See Also:**
    ...
    """
    if delimiter_regex and isinstance(delimiter_regex, bytes):
        import re
    ...
    if linefixer or delimiter_regex:
        ...
        F = fh.read().strip('\n').split('\n')
        if linefixer:
            F = list(map(linefixer,F))
        if delimiter_regex:
            F = [delimiter_regex.sub(metadata['dialect'].delimiter, line) for line in F]
        fh2.write('\n'.join(F))
        fh2.seek(0)
        fh = fh2
...
def getmetadata(fname, inflines=2500, linefixer=None, delimiter_regex=None,
    ...
    metadata = dict([(k,v) for (k,v) in list(metadata.items()) if v is not None])

    if comments is None:
        comments = '#'

    if 'metametadata' in list(metadata.keys()):
        mmd = metadata['metametadata']
    else:
        mmd = None

    if 'formats' in list(metadata.keys()) and is_string_like(metadata['formats']):
    ...
    if storedmetadata:
        ...
        for name in storedmetadata:
            if ((name in list(metadata.keys())) and (storedmetadata[name] != None) and
    ...
    if inflines is None:
        ...
        if 'headerlines' in list(metadata.keys()):
    ...
    if 'headerlines' not in list(metadata.keys()):
    ...
    if 'dialect' not in list(metadata.keys()):
        if 'delimiter' not in list(metadata.keys()):
    ...
    if 'names' not in list(metadata.keys()) and namesinheader and not storedmetadata:
...
def printdelimitercheckmessage(dialect,infdia,storedmetadata,verbosity):
    if infdia.delimiter != dialect.delimiter:
        if verbosity >= 5:
            if (storedmetadata and ('delimiter' in list(storedmetadata.keys())) and
...
def metadatacolthreshold(metadata, getcols):
    ...
    if 'types' in list(metadata.keys()):
        f = metadata['types'][:]
...
def inferdialect(fname=None, datalines=None, delimiter_regex=None,
    ...
    **Returns**
    ...
    if delimiter_regex:
        ...
        for m in matches:
            for x in set(m):
                poss[x] = m.count(x) + (poss[x] if x in list(poss.keys()) else 0)
        MaxVal = max(poss.values())
        assert MaxVal > 0, 'delimiter_regex found no matches'
        amax = [x for x in list(poss.keys()) if poss[x] == MaxVal][0]
...
def readstoredmetadata(fname, skiprows=0, linenumber=None, comments='#',
    ...
    if metametadata:
        ...
        Tval = max([v if isinstance(v,int) else max(v)
                    for v in list(metametadata.values())])
        fh = file(fname, 'rU')
        data = [fh.readline() for _ind in range(Tval + 1 + skiprows)][skiprows:]

        if (max([v if isinstance(v,int) else max(v)
                for v in  list(metametadata.values())]) < len(data)):
            for n in list(metametadata.keys()):
...
def processmetadata(metadata, items=None, comments=None, delimiter_regex=None,
   ...
   **Returns**
    ...
    if comments is None:
        if 'comments' in list(metadata.keys()):
        ...
        if (('comments' in list(metadata.keys())) and
    ...
    if not items or 'dialect' in items:
        if 'dialect' in list(metadata.keys()):
            ...
            if 'delimiter' in dir(metadata['dialect']):
                for a in dir(metadata['dialect']):
                    if not a.startswith('_') and a in list(metadata.keys()):
                    ...
                    elif not a.startswith('_') and a not in list(metadata.keys()):
    ...
    if (not items or 'names' in items) and ('names' in list(metadata.keys())):
        if is_string_like(metadata['names']):

            if delimiter_regex:
                metadata['names'] = delimiter_regex.split(metadata['names'])
            elif (('dialect' in list(metadata.keys())) and
    ...
    if (not items or 'formats' in items) and 'formats' in list(metadata.keys()):
        if is_string_like(metadata['formats']):
            if delimiter_regex:
                metadata['formats'] = delimiter_regex.split(metadata['formats'])
            elif (('dialect' in list(metadata.keys())) and ('delimiter' in dir(metadata['dialect']))):
                d = metadata['dialect']
                n = metadata['formats']
    ...
    if (not items or 'types' in items) and 'types' in list(metadata.keys()):
    ...
    if (not items or 'coloring' in items) and ('coloring' in list(metadata.keys())):
...
def inferheader(lines, comments=None, metadata=None,
    ...
    **Returns**
    ...
    if ((comments is None) and metadata and ('comments' in list(metadata.keys()))):
        comments = metadata['comments']
    if (comments is None):
        comments = '#'
...
def isctype(x, t):
    ...
    b = t == bool and isinstance(x, Name) and \
                     x.name in ['False', 'True']
    c =  (t == type(None) and isinstance(x, Name) and x.name == 'None')
    return a or b or c
