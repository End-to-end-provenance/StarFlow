    ...
    if Simple or IsFile(path):
        if HoldTimes == None or not path in list(HoldTimes.keys()):
            ...
            if not IsFile(path) or objectname == '' or not (path.split('.')[-1] == 'py' and objectname.startswith(pname + '.')):
                ...
                if StoredModuleTimes != None and objectname in list(StoredModuleTimes.keys()):
...
def ListFindMtimes(FileParts,HoldTimes = None,Simple=True,
    ...
    for (K,i) in A:
        ...
        if PathExists(StoredModuleTimesPath):
            ...
            On = numpy.array(list(range(i,next)))[E]
...
def GetNestedObject(name,Members):
    ...
    if name in list(Members.keys()):
        return Members[name]
    else:
        for mname in list(Members.keys()):
    ...
    for path in paths:
        if PathExists(path):
            ...
            if not PathExists(StoredTimesPath):
                ...
                try:
                    StoredTimes = pickle.load(open(StoredTimesPath,'rb'))
                except:
                    Remake = True
                else:
                    if not PathExists(StoredModulePath) or not isinstance(StoredTimes,dict) or not '__hash__' in list(StoredTimes.keys()):
                        Remake = True
                    else:
            ...
            if Force or Remake or  os.path.getmtime(path) > os.path.getmtime(StoredTimesPath):
                if Remake:
                    ...
                    StoredModule = pickle.load(open(StoredModulePath,'rb'))
                ...
                try:
                    ...
                    MissingParts = set(StoredModule.keys()).difference(list(Parts.keys()))
                    ...
                    for p in list(Parts.keys()):
                        ...
                        if p in list(StoredModule.keys()):
...
class StoredModulePart():
    ...
    def reconstitute(self):
        if self.loadmethod in [None,'']:
            ...
            return dict([(p,self.content[p].reconstitute()) if 'reconstitute' in dir(self.content[p]) else (p,self.content[p])  for p in list(self.content.keys())])
        else:
...
def ClassMethodDumps(obj):
    ...
    '''
    return {'__classname__':obj.__self__.__class__.__name__, 'im_func': StoredModulePart(obj.__func__)}
...
def FunctionDumps(obj):
    ...
    A = {'__name__':obj.__name__,'__module__':obj.__module__,'func_code':StoredModulePart(obj.__code__),'func_defaults':StoredModulePart(obj.__defaults__)}
    if obj.__dict__ != {}:
        #A['func_dict'] = dict([(k,StoredModulePart(v)) for (k,v) in obj.func_dict.items()])
         A['func_dict'] = StoredModulePart(obj.__dict__,recursive = True,ScopeName=obj.__module__)
...
def StoredDocstring(path):
    ...
    if IsDotPath(path):
        ...
        if StoredModule != None:
            if Fnname in list(StoredModule.keys()):
        ...
        if StoredModule != None:
            if '__doc__' in list(StoredModule.keys()):
